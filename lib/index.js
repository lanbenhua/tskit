...
var fsPromise__default = /*#__PURE__*/_interopDefaultLegacy(fsPromise);
...
var TEMPLATES_DIR = path__default["default"].resolve(__dirname, "../templates");
var getTemplateDir = function (template) {
    return path__default["default"].resolve(TEMPLATES_DIR, template);
};
var copy = function (fromDir, toDir) {
    if (!isExists(fromDir))
        throw new Error("".concat(fromDir, " is not exists."));
    if (!isDirectory(fromDir))
        throw new Error("".concat(fromDir, " is not a directory."));
    if (!isExists(toDir)) {
        fs__default["default"].mkdirSync(toDir);
        console.log(source.green("[tskit] created '".concat(toDir, "'")));
    }
    if (!isDirectory(toDir))
        throw new Error("".concat(toDir, " is not a directory."));
    fsPromise__default["default"].readdir(fromDir).then(function (subFilenames) {
        subFilenames.forEach(function (filename) {
            var fromPath = path__default["default"].resolve(fromDir, filename);
            var toPath = path__default["default"].resolve(toDir, filename);
            if (isDirectory(fromPath))
                return copy(fromPath, toPath);
            if (isFile(fromPath)) {
                fs__default["default"].openSync(toPath, fs__default["default"].constants.O_CREAT | fs__default["default"].constants.O_TRUNC, 511);
                console.log(source.green("[tskit] created '".concat(toPath, "'")));
                fs__default["default"].createReadStream(fromPath, { mode: 511 }).pipe(fs__default["default"].createWriteStream(toPath, { mode: 511 }));
            }
        });
    });
};
var init = function (dir, template, force) {
    var fullpath = path__default["default"].resolve(process.cwd(), dir);
    var isCurrentDirExists = isExists(fullpath);
    if (!force && isCurrentDirExists)
        throw new Error("".concat(fullpath, " exists, you can use '--force' or '-f' to force clear the directory."));
    if (force && isCurrentDirExists) {
        console.log(source.green("[tskit] exists '".concat(fullpath, "'")));
        fs__default["default"].rmdirSync(dir, { recursive: true });
        console.log(source.green("[tskit] removed '".concat(fullpath, "'")));
    }
    var templateDir = getTemplateDir(template);
