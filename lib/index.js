...
var copy = function (fromDir, toDir) {
    ...
    fsPromise__default["default"].readdir(fromDir).then(function (dirChildren) {
        dirChildren.forEach(function (dirChild) {
            var fromPath = path__default["default"].resolve(fromDir, dirChild);
            var toPath = path__default["default"].resolve(toDir, dirChild);
...
var init = function (dir, template, force) {
    var fullDirPath = path__default["default"].resolve(process.cwd(), dir);
    var isDirExists = isExists(fullDirPath);
    if (!force && isDirExists)
        throw new Error("".concat(fullDirPath, " exists, you can use '--force' or '-f' to force clear the directory."));
    if (force && isDirExists) {
        console.log(source.green("[tskit] exists '".concat(fullDirPath, "'")));
        fs__default["default"].rmdirSync(dir, { recursive: true });
        console.log(source.green("[tskit] removed '".concat(fullDirPath, "'")));
    }
